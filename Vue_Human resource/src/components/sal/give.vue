 ...
 <el-card >
     <!-- 搜索与添加区域 -->
        <el-row :gutter="20">
            ...
            <el-col :span="4">
                <el-button type="primary" @click="submit">提交成功页面</el-button>
            </el-col>
        </el-row>
        ...
        <el-table :data="userlist" border stripe>
            ...
            <el-table-column label="基础薪资" prop="money"></el-table-column>
            <el-table-column label="标准审批数量" prop="count"></el-table-column>
                ...
                <template slot-scope="scope">
                        ...
                        <!-- 分配角色按钮 -->
                         <el-tooltip  effect="dark" content="查看详情" placement="top" :enterable="false">
                           <el-button type="warning" icon="el-icon-setting"  size="mini" @click="showEditDialog(scope.row.o3_id)"></el-button>
                         </el-tooltip>
                </template>
            </el-table-column>
        </el-table>
...
admin:window.sessionStorage.getItem("username")
 ...
 methods:{
   ...
   async getUserList(){
      ...
      const {data:res} = await this.$http.post(`/Sal/give/giveSalInfo/${this.queryInfo.pagenum}/${this.queryInfo.pagesize}`)
...
// this.$router.push('/sal/giveList')
    const {data:res}= await this.$http.post('/Sal/give/updateSalGiveInfoByO3Id/'+id)
