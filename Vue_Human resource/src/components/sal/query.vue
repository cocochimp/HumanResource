 ...
 <el-card >
     <!-- 搜索与添加区域 -->
        <el-row :gutter="20">
            <el-col :span="10" style="margin:10px 0">
                <!-- clearable添加了为可清空的文本框 -->
                 <el-input placeholder="薪资标准"  class="input-with-select" v-model="queryInfo.query1" clearable @clear="getUserList">
                    <el-button slot="append" icon="el-icon-search" @click="getUserListByName1"></el-button>
                 </el-input>
            </el-col>
           
            <el-col :span="10" style="margin:20px 0 ;color: white;">
             ...
             <el-col :span="10" >
               <el-input placeholder="登记人查询"  class="input-with-select" v-model="queryInfo.query3" clearable @clear="getUserList">
        ...
        <el-table :data="userlist" border stripe>
            ...
            <el-table-column label="名字" prop="f_name"></el-table-column>
            <!-- <el-table-column label="所属一级机构" prop="o1_name"></el-table-column> -->

            <el-table-column label="登记人" prop="username"></el-table-column>
            ...
            <el-table-column label="操作">
                <template slot-scope="scope">
                        <!-- 修改按钮 -->
                        <el-button type="success" icon="el-icon-zoom-in" size="mini" @click="showEditDialog(scope.row.s_id)"></el-button>
                        <!-- 删除按钮 -->
...
admin:window.sessionStorage.getItem("username")
 ...
 methods:{
   ...
   async getUserList(){
      ...
      const {data:res} = await this.$http.post(`/Sal/query/showSalSelect/${this.queryInfo.pagenum}/${this.queryInfo.pagesize}`)
    ...
    //   }
      ...
      this.total=8
      window.sessionStorage.setItem('id',res.data[0].s_id)
   ...
   //获取一级权限分类列表
    ...
    async getUserListByName1(){
       this.queryInfo.pagenum=0
      const {data:res} = await this.$http.post(`/Sal/query/showSalSelectByXinZiBiaoZhun/${this.queryInfo.pagenum}/${this.queryInfo.pagesize}/${this.queryInfo.query1}`)
    //   if(res.meta.status!==200){
    //       return this.$message.error('获取用户列表失败！')
    //   }
    ...
    //   }
     ...
     async getUserListByName3(){
       this.queryInfo.pagenum=0
      const {data:res} = await this.$http.post(`/Sal/query/showSalSelectByDengJiRen/${this.queryInfo.pagenum}/${this.queryInfo.pagesize}/${this.queryInfo.query3}`)
    //   if(res.meta.status!==200){
    //       return this.$message.error('获取用户列表失败！')
    //   }
      console.log(res.data);
