     ...
     <!-- 搜索与添加区域 -->
        ...
        <el-table :data="userlist" border stripe>
            ...
            <el-table-column label="操作">
                <template slot-scope="scope">
                        <!-- 修改按钮 -->
                        <el-button type="primary" icon="el-icon-edit" size="mini" @click="showEditDialog(scope.row.f_id)"></el-button>
                        <!-- 删除按钮 -->
                        <el-button type="danger" icon="el-icon-delete"  size="mini" @click="removeUserById(scope.row.p_id)"></el-button>
                        <!-- 分配角色按钮 -->
        ...
        <el-dialog
            ...
            <el-form ref="editFormRef" :model="editForm" :rules="editFormRules" label-width="100px" >
                <!-- prop是绑定验证规则的属性 -->
                     ...
                     <el-image
      style="width: 180px; height: 180px; transform:translateX(150%)"
 ...
 methods:{
   ...
   async getUserList(){
    //    this.queryInfo.pagenum=0
      const {data:res} = await this.$http.post(`/File/check/showFileRechecked/${this.queryInfo.pagenum}/${this.queryInfo.pagesize}`)
...
this.editDialogVisible = false
 ...
 editUserInfo(){
     this.$refs.editFormRef.validate(async valid=>{
         if(!valid) return
         //发起修改用户信息的数据请求
         const {data:res} =await this.$http.post(`/File/check/updateCheckInfoByFid/${window.sessionStorage.getItem('id')}`)
